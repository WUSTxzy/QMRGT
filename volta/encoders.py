...
class Graphaggregation(nn.Module):
    def __init__(self, config):
        ...
        self.w_g1 = nn.Linear(config.graph_common_dim*2, config.graph_common_dim)
        self.w_g2 = nn.Linear(config.graph_common_dim, config.graph_common_dim)
        self.p_g = nn.Linear(config.graph_common_dim, 1)
        # weight distribution
...
class TextContext(nn.Module):
    def __init__(self, config):
        ...
        self.P = nn.Linear(config.graph_common_dim, 1)
        self.softmax = nn.Softmax(dim=-2)

    def forward(self, textual_feats):
        # textual_feats [256, 26, 1024]
        z_h = self.P(textual_feats)   # [256, 26, 1]
        h = self.softmax(z_h)  # [256, 26, 1]
        c = torch.matmul(h.transpose(-1, -2), textual_feats)  # [256,1,1024]
        return c
...
class ReasoningInstructionExtraction(nn.Module):
    def __init__(self, config):
        ...
        self.fc1 = nn.Linear(config.graph_common_dim, config.graph_common_dim)
        self.fc2 = nn.Linear(config.graph_common_dim, config.graph_common_dim)
        self.relu = nn.LeakyReLU()
        self.F = nn.Linear(config.graph_common_dim, 1)  # d*1
        self.softmax = nn.Softmax(dim=-2)
        self.fc2 = nn.Linear(config.graph_common_dim*2, config.graph_common_dim)

    def forward(self, questions, instruction, history_vector):
        W_q = l2norm(self.fc2(instruction + self.relu(self.fc1(questions))))
        a_q = self.softmax(self.F(W_q))
        s_g = torch.matmul(a_q.transpose(-1, -2), W_q) + history_vector
        return s_g
...
class HistoryVectorExtraction(nn.Module):
    def __init__(self, config):
        ...
        self.fc1 = nn.Linear(config.graph_common_dim * 2, config.graph_common_dim)
        self.relu = nn.LeakyReLU()
        self.fc2 = nn.Linear(config.graph_common_dim, config.graph_common_dim)
        self.F = nn.Linear(config.graph_common_dim * 2, 1)  # d*1
        self.softmax = nn.Softmax(dim=-2)

    def forward(self, v_feats):
        W_v = l2norm(self.fc2(self.relu(self.fc1(v_feats))))  # [256,37,1024 *2]
        a_v = self.softmax(self.F(W_v))  # [256, 37, 1]
        s_v = torch.matmul(a_v.transpose(-1, -2), W_v)  # [256, 1, 1024 *2]
...
class GNN(nn.Module):
    def __init__(self, config):
        ...
        self.common_dim = config.graph_common_dim
        self.ins_extr = ReasoningInstructionExtraction(config)
        self.history_vector_predictor = HistoryVectorExtraction(config)
        self.steps = Max_step  # 3
        self.ins_layers = nn.ModuleList(
            [copy.deepcopy(self.ins_extr) for _ in range(self.steps)]
        )
        self.w1 = nn.Linear(config.graph_common_dim * 2, config.graph_common_dim)
        self.w2 = nn.Linear(config.graph_common_dim * 2, config.graph_common_dim)
        self.w3 = nn.Linear(config.graph_common_dim , config.graph_common_dim)

        # fc update vertex feat
        self.w4 = nn.Linear(config.graph_common_dim * 2, config.graph_common_dim)
        self.w5 = nn.Linear(config.graph_common_dim, config.graph_common_dim)
        self.w6 = nn.Sequential(
            nn.Linear(config.graph_common_dim * 3, config.graph_common_dim * 2),
            nn.ReLU(),
            nn.Linear(config.graph_common_dim * 2, config.graph_common_dim * 2)
        )
        self.dropout = nn.Dropout(0.1)
    ...
    def forward(self, v_feats, embedding_output):
        ...
        for i in range(self.steps):
            ...
            history_vector_list.append(attention_map)
...
class GraphReasoning(nn.Module):
    def __init__(self, config):
        ...
        self.image_linear = nn.Linear(config.v_hidden_size, config.graph_common_dim)
        self.text_linear = nn.Linear(config.hidden_size, config.graph_common_dim)
        self.question_embedding = nn.Linear(config.hidden_size, config.graph_common_dim)
        self.textual_context = TextContext(config)
        self.gnn = GNN(config)
        self.graph_attention_emb = Graphaggregation(config)
    ...
    def forward(self, images, texts, embedding_output):
        ...
        return graph_emb_v, graph_emb_t
        # return final_output
...
class BertModel(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.use_graph_reasoning = config.graph_reasoning
        self.graph_reasoning = GraphReasoning(config)
    ...
    def forward(self, input_txt, input_imgs, image_loc, token_type_ids=None, attention_mask=None,
        ...
        # print("input_txt", input_txt.shape)  # input_txt torch.Size([256, 26])
        # print("input_imgs", input_imgs.shape)  # input_imgs torch.Size([256, 37, 2048])
        # print('image_loc', image_loc.shape)  # [128,37,5]
        # print("attention_mask", attention_mask.shape)  # attention_mask torch.Size([256, 26])
        # print("token_type_ids", token_type_ids.shape)  # token_type_ids torch.Size([256, 26])
        # print("image_attention_mask", image_attention_mask.shape)  # image_attention_mask torch.Size([256, 37])
        # print('output_all_encoded_layers', output_all_encoded_layers)  # false
        # print('output_all_attention_masks', output_all_attention_masks)  # false
        ...
        if self.shared_embeddings:
            ...
            embedding_output = self.embeddings(input_txt, token_type_ids)  # [256, 26, 768]
        ...
        sequence_output_t = encoded_layers_t[-1]  # [256, 26, 768]
        sequence_output_v = encoded_layers_v[-1]  # [256, 37, 1024]

        if self.use_graph_reasoning:
            final_output_v, final_output_t = self.graph_reasoning(sequence_output_v, sequence_output_t, embedding_output)
        else:
            if self.fusion_method == "vl-bert_vqa":
                text_mask = input_txt != 0
                text_end = text_mask.sum(1, keepdim=True)
                final_output_t = self.t_pooler(sequence_output_t, text_end)
            else:
                final_output_t = self.t_pooler(sequence_output_t)
            final_output_v = self.v_pooler(sequence_output_v)
        ...
        return encoded_layers_t, encoded_layers_v, final_output_t, final_output_v, all_attention_mask
