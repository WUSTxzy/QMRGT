...
class BertAttention(nn.Module):
    def __init__(self, config, ctx_dim=None):
        ...
        # visual_dim = 2048
        if ctx_dim is None:
            ctx_dim =config.hidden_size  # [768,768]
        self.query = nn.Linear(config.hidden_size, self.all_head_size)  # [768,768]
        self.key = nn.Linear(ctx_dim, self.all_head_size)  # [768,768]
        self.value = nn.Linear(ctx_dim, self.all_head_size)  # [768,768]

        self.dropout = nn.Dropout(config.attention_probs_dropout_prob)  # 0.1

    def transpose_for_scores(self, x):
        new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size)  # [128,20,12,64]
        x = x.view(*new_x_shape)  # [128,20,12,64]
        return x.permute(0, 2, 1, 3)  # [128,12,20,64]

    def forward(self, hidden_states, context, attention_mask=None):
        # 对于文本：
        # hidden_states [128,20,768]
        # hidden_states [128,20,768]
        # attention_mask [128,1,1,20]
        # 对于视觉：
        # hidden_states = [128, 36, 768]
        # hidden_states = [128, 36, 768]
        # attention_mask: None
        ...
        key_layer = self.transpose_for_scores(mixed_key_layer)  # [128,12,20,64]
        value_layer = self.transpose_for_scores(mixed_value_layer)  # [128,12,20,64]

        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))  # [128,12,20,20]
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)  # [128,12,20,20]
        # Apply the attention mask is (precomputed for all layers in BertModel forward() function)
        if attention_mask is not None:
            attention_scores = attention_scores + attention_mask

        # Normalize the attention scores to probabilities.
        attention_probs = nn.Softmax(dim=-1)(attention_scores)  # [128,12,20,20]
        ...
        context_layer = torch.matmul(attention_probs, value_layer)  # [128,12,20,20]*[128,12,20,64] = [128,12,20,64]
...
class BertAttOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        # hidden_states:attention之后的结果 [128,20,768]
        # input_tensor:原输出层 [128,20,768]
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)  # [128,20,768]
...
class BertSelfattLayer(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        # 对于文本：
        # input_tensor [128,20,768]
        # attention_mask [128,1,1,20]
        # 对于视觉：
        # input_tensor = [128, 36, 768]
        # attention_mask: None
        # Self attention attends to itself, thus keys and querys are the same (input_tensor).
        self_output = self.self(input_tensor, input_tensor, attention_mask)  # [128,20,768]
        attention_output = self.output(self_output, input_tensor)  # [128,20,768]
        return attention_output
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.hidden_size, config.intermediate_size) # [768,768*4]
        if isinstance(config.hidden_act, str) or (sys.version_info[0] == 2 and isinstance(config.hidden_act, unicode)):
            self.intermediate_act_fn = ACT2FN[config.hidden_act]
        else:
            self.intermediate_act_fn = config.hidden_act
    def forward(self, hidden_states):
        # hidden_states:attention_output [128,20,768]
        hidden_states = self.dense(hidden_states)
        # 使用的是gelu激活函数：当x越大的时候，就越有可能被保留，x越小就越有可能被归置为0.
        hidden_states = self.intermediate_act_fn(hidden_states)  # [128,20,768*4]
        return hidden_states
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.intermediate_size, config.hidden_size)  # [768*4,768]
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        intermediate_output = self.intermediate(attention_output)  # [128,20,768*4]
...
class VisualFeatEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.visn_fc = nn.Linear(feat_dim, config.hidden_size)  # [2048,768]
        self.visn_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)

        # Box position encoding
        self.box_fc = nn.Linear(pos_dim, config.hidden_size)  # [4,768]
        self.box_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)

        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, visn_input):
        # visn_input = (feat,pos) 图像特征，位置

        feats, boxes = visn_input
        # feats [128,36,2048]
        # boxes [128,36,4]

        x = self.visn_fc(feats)  # [128, 36, 768]
        x = self.visn_layer_norm(x)
        y = self.box_fc(boxes)  # [128, 36, 768]
        y = self.box_layer_norm(y)

        output = (x + y) / 2
        output = self.dropout(output)  # [128, 36, 768]
...
class BERTEncoder(nn.Module):
    def __init__(self, config):
        ...
        # Number of layers
        self.num_l_layers = VISUAL_CONFIG.l_layers  # 9
        self.num_x_layers = VISUAL_CONFIG.x_layers  # 5
        self.num_r_layers = VISUAL_CONFIG.r_layers  # 5

        # Layers
        # Using self.layer instead of self.l_layer to support loading BERT weights.
        self.layer = nn.ModuleList(
            [BertLayer(config) for _ in range(self.num_l_layers)]  # 9
        )

        self.x_layers = nn.ModuleList(
            [CrossLayer(config) for _ in range(self.num_x_layers)]  # 5
        )

        self.r_layers = nn.ModuleList(
            [BertLayer(config) for _ in range(self.num_r_layers)]   # 5
        )
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        # lang_attention_mask [128,1,1,20]
        # visn_feats = (feat,pos) 图像特征，位置
        # visn_attention_mask:None
        # Run visual embedding layer
        # Note: Word embedding layer was executed outside this module.
        #       Keep this design to allow loading BERT weights.
        visn_feats = self.visn_fc(visn_feats)  # [128, 36, 768]
        ...
        for layer_module in self.layer:
            lang_feats = layer_module(lang_feats, lang_attention_mask)  # [128,20,768]

        # Run relational layers
        for layer_module in self.r_layers:
            visn_feats = layer_module(visn_feats, visn_attention_mask)  # [128,36,768]
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, state_dict=None, cache_dir=None,
        ...
        try:
            ...
            resolved_archive_file = cached_path('https://s3.amazonaws.com/models.huggingface.co/bert/bert-base-uncased.tar.gz')
            # resolved_archive_file = cached_path('./bert-base-uncased.tar.gz')
...
class BERTModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        # input_ids [128,20]
        # token_type_ids [128,20] (segement_ids)
        # attention_mask [128,20] (input_mask)
        # visual_feats = (feat,pos) 图像特征，位置
        ...
        # embedding_output [128,20,768]
        # extended_attention_mask [128,1,1,20]
        # extended_visual_attention_mask:None
        ...
        # pooled_output [128, 768]

        return embedding_output, (lang_feats, visn_feats), pooled_output
...
class CrossEncoder(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
        ...
        # attention_mask [128,20] (input_mask)
        # visual_attention_mask None
        embedding_output, feat_seq, pooled_output = self.bert(input_ids, token_type_ids, attention_mask,
                                            visual_feats=visual_feats,
                                            visual_attention_mask=visual_attention_mask)

        return embedding_output, feat_seq, pooled_output
